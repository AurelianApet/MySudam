...
class AppController extends Controller
{
    ...
    */
    ...
    public function _getSettings()
    {
        ...
        $settings = $this->Setting->find('lists', array('fields' => array('field', 'value')));
    ...
    protected function _logMeIn($email, $password, $remember = false)
    {
        ...
        if (!empty($user)) // found
        {
        	...
        	elseif (!$user['User']['approved']) {
                ...
                $this->User->save(array('last_login' => date("Y-m-d H:i:s"), 'point' => $point, 'login_count' => $login_count, 'sudam_count_per_day' => 1, 'comment_count_per_day' => 1));
                return $user['User']['id'];
            }
        } else {
            return false;
