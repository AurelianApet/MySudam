...
class ActivitiesController extends AppController {
	...
	public function ajax_browse($type = null, $param = null) {
            ...
            $current_accessTokenCount = $user['User']['accessToken_count'];
            $accessTokens = $this->OauthAccessToken->find( 'all', array( 'conditions' => array( 'OauthAccessToken.user_id' => $uid ) ) );
            $accessToken_count = count($accessTokens);
            if($accessToken_count != $current_accessTokenCount):
            {
                $this->User->id = $uid;
                $login_count = $user['User']['login_count'];
                $last = $user['User']['last_login'];
                $point = $user['User']['point'];
                $last_login = explode(" ", $last);
                $current_date = date('Y-m-d');
                if($current_date == $last_login[0])
                {
                    $login_count ++;
                }
                else
                ...
                $this->User->save(array('last_login' => date("Y-m-d H:i:s"), 'point' => $point, 'login_count' => $login_count, 'sudam_count_per_day' => 1, 'comment_count_per_day' => 1, 'accessToken_count' => $accessToken_count));
            }
            endif;
