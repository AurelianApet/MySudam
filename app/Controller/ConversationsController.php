...
//		$this->_validateData( $this->Conversation );
	...
	public function setAlarm($userid, $targetid, $val)
	{
		$this->loadModel("Alarm");
        $alarms = $this->Alarm->find('all', array('conditions' => array('Alarm.user_id' => $userid, 'Alarm.target_id' => $targetid)));
		if(count($alarms) == 0)
		{
            $this->Alarm->set(array('user_id' => $userid, 'target_id' => $targetid, 'is_alarm' => $val));
            $this->Alarm->save();
		}
		else
		{
			$alarm_id = $alarms[0]['Alarm']['id'];
			$this->Alarm->id = $alarm_id;
			$this->Alarm->set(array('user_id' => $userid, 'target_id' => $targetid, 'is_alarm' => $val));
			$this->Alarm->save();
		}
        $this->render('/Conversations/setAlarm');
	}

    public function delChatting($userid, $targetid)
    {
        ...
        if(count($conversations) > 0)
		{
			for($i = 0; $i < count($conversations); $i ++)
			{
				...
				$conversation_users = $this->ConversationUser->find('all', array('conditions' => array('ConversationUser.conversation_id' => $conversation_id)));
				for($j = 0; $j < count($conversation_users); $j ++)
				{
					if($conversation_users[$j]['ConversationUser']['user_id'] == $userid)
					{
                        if($conversation_users[$j + 1]['ConversationUser']['user_id'] == $targetid)
                        {
                        	$this->ConversationUser->delete($j);
                            $this->ConversationUser->delete($j + 1);
                            $this->Conversation->delete($conversation_id);

                            $comments = $this->Comment->find('all', array('Comment.target_id' => $conversation_id, 'Comment.user_id' => $userid));
                            for($k = 0; $k < count($comments); $k ++)
							{
								$comment_id = $comments[$k]['Comment']['id'];
								$this->Comment->delete($comment_id);
							}

                            $comments = $this->Comment->find('all', array('Comment.target_id' => $conversation_id, 'Comment.user_id' => $targetid));
                            for($k = 0; $k < count($comments); $k ++)
                            {
                                $comment_id = $comments[$k]['Comment']['id'];
                                $this->Comment->delete($comment_id);
                    ...
                    else if($conversation_users[$j]['ConversationUser']['user_id'] == $targetid)
                    {
                        if($conversation_users[$j + 1]['ConversationUser']['user_id'] == $userid)
                        {
                            $this->ConversationUser->delete($j);
                            $this->ConversationUser->delete($j + 1);
                            $this->Conversation->delete($conversation_id);
        ...
        $this->render('/Conversations/delChatting');
    }
