...
<?php
    ...
    {
        $user_follow_id = $followers[$i]['UserFollow']['user_follow_id'];
        $cuser = $userModel->find( 'first', array( 'conditions' => array( 'User.id' => $user_follow_id ) ) );
        if(empty($cuser))
            continue;
...
?>
    <a href = "<?php echo $this->Html->url('/', true); ?>users/view/<?php echo $user_follow_id; ?>">
        <table width="100%">
             <tr>
                <td width="20%">
