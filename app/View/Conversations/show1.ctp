...
<?php else: ?>
    <?php
     ...
     foreach ($conversations as $conversation):
        ...
        $conversationUsers = $conversationuserModel->find( 'all', array( 'conditions' => array( 'ConversationUser.conversation_id' => $conversation_id ) ) );
    ...
    <li <?php echo $conversation['ConversationUser']['unread'] ? 'class="unread"' : ''?>>
        <a href="<?php echo $this->request->base?>/conversations/view/<?php echo $conversation['Conversation']['id']?>">
            <?php
//             echo $this->Moo->getImage(array('User' => $conversation['Conversation']['LastPoster']),array( "width" => "45px", "class" => "img_wrapper2", 'prefix' => '50_square'))
            ?>
