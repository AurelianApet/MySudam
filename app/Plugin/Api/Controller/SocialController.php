...
define('SOCIAL_KAKAO', 'kakao');
define('SOCIAL_NAVER', 'naver');
...
class SocialController extends ApiAppController
{
    ...
    public function auth()
    {
        ...
        if (!empty($user))
        {
            ...
            {
                //같은 이메일 주소이지만 provider 가 다르면 새로운 계정임
                if($petID == "")
                {
                    //클라쪽에서 로그인시도, 실패 등록되지 않은 계정
                    $this->set(array(
                        'message' => __d('api', 'failure'),
                        'confirm_password' => true,
                        '_serialize' => array('message', 'not_signed'),
                    ));
                }
                else
                {
                ...
                {
                    //클라쪽에서 회원가입시도, 실패 이미 존재하는 계정
                    $this->set(array(
                        'message' => __d('api', 'failure'),
                        'confirm_password' => true,
                        '_serialize' => array('message', 'already_existing'),
                    ));
                }
            }
        }
    ...
    private function saveSocialUser($user_id, $provider_user, $verify_email = false)
    {
        ...
        if($this->SocialUser->save())
        {
            ...
            //add pet to the companion table first
            $companion['name'] = $this->request->data['name'];
            $companion['ani_type'] = $this->request->data['ani_type'];
            $companion['type'] = $this->request->data['type'];
            $companion['birthday'] = $this->request->data['birthday'];
            $companion['gender'] = $this->request->data['petgender'];

            $this->Companion->set($companion);
            $id = 0;
            if($this->Companion->save())
    ...
    {
        $social_list = array(SOCIAL_FACEBOOK, SOCIAL_GOOGLE, SOCIAL_KAKAO, SOCIAL_NAVER);
