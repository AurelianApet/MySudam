...
class BlogsController extends BlogAppController
{
    ...
    public function create($id = null)
    {
        ...
        if (!empty($id)) // editing
        {
            ...
            $this->set('title_for_layout', __('Write Blog'));
        }
    ...
    public function save()
    {
        ...
        $like_id = $this->request->data['like_id'];
        $dislike_id = $this->request->data['dislike_id'];
    ...
    public function saveSudam()
    {
        ...
        $this->Stroke->save(array('user_id' => $userid, 'target_id' => $targetid, 'response' => $response, 'message' => $message, 'blog_id' => $blogid));
        $sid = $this->Stroke->find('first', array('order' => array('Stroke.id' => 'desc')));

        $this->loadModel('Blog.Blog');
        $blog = $this->Blog->find('first', array('conditions' => array('Blog.id' => $blogid)));
        $scount = $blog['Blog']['stroke_count'];

        $scount ++;
        $this->Blog->id = $blogid;
        $this->Blog->set(array('stroke_count' => $scount, 'stroke_id' => $sid['Stroke']['id']));
        $this->Blog->save();

        $this->loadModel('User');
        $user = $this->User->find('first', array('conditions' => array('User.id' => $userid)));
        $sudam_count_per_day = $user['User']['sudam_count_per_day'];
        $sudam_count_per_day ++;

        $comment_count_per_day = $user['User']['comment_count_per_day'];

        $point = $user['User']['point'];
        if($sudam_count_per_day < 11)
        {
            $point ++;
        }
        ...
        $this->User->id = $userid;
        $this->User->set(array('point' => $point, 'sudam_count_per_day' => $sudam_count_per_day, 'comment_count_per_day' => $comment_count_per_day));
        $this->User->save();

        $this->render('Blog.Blogs/saveSudam');
    }
}
?>
