...
class VideoApiListener implements CakeEventListener
{
    ...
    function getVideoInfo($id,$v){
        ...
        if (!empty($object['Video']['group_id'])) {
            ...
            $url = FULL_BASE_URL . $v->request->base . '/videos/view/' . $object['Video']['id'] . '/' . seoUrl($object['Video']['title']);
        }
        $objectsExport =  array(
            'type' => 'Video_Video',
            'id' => $object["Video"]['id'],
    ...
    function exportVideoCreate($e)
    {
        ...
        if ($data['Activity']['target_id']) {
            ...
            $show_subject = MooCore::getInstance()->checkShowSubjectActivity($object);
    ...
    function exportVideoActivity($e)
    {
        ...
        $activityTextHtml = nl2br($v->Moo->parseSmilies($v->Text->autoLink($data['Activity']['content'], array_merge(array('target' => '_blank', 'rel' => 'nofollow', 'escape' => false),array('no_replace_ssl' => 1)))));
        if (!empty($data['UserTagging']['users_taggings'])) :
                ...
                $activityTextHtml = nl2br($v->Moo->parseSmilies($v->Text->autoLink($data['Activity']['content'], array_merge(array('target' => '_blank', 'rel' => 'nofollow', 'escape' => false),array('no_replace_ssl' => 1))))) . $v->MooPeople->with($data['UserTagging']['id'], $data['UserTagging']['users_taggings'], false);
