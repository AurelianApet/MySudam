...
var ChatCounterUnseenMesasgeActionCreators = require('../actions/ChatCounterUnseenMesasgeActionCreators');
// Mobi
...
function getUsersByRoomIdsAtBootingCallback(data) {
    ...
    for(var i =0;i<roomsIsOpened.isCreated.length;i++){
        module.exports.sendRequestCreateChatWindowByRoomId(roomsIsOpened.isCreated[i],roomsIsOpened[roomsIsOpened.isCreated[i]].minimized,CHAT_CONSTANTS.NOT_FOCUSED_CHAT_WINDOW); // m min minimized
    }
}
...
module.exports = {
    ...
    initSocket: function (callback) {
        ...
        window.test = _mySocket;

    },

    createChatWindowForAUser: function (uId) {
        console.log("Create chat for user");
