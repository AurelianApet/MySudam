...
class ChatsController extends ChatAppController{
    ...
    public function delChatting($userid, $targetid)
    {
        ...
        $chatrooms = $this->ChatRoom->find('first', array('conditions'=>array('ChatRoom.code' => $roomid)));
        $this->ChatRoom->delete($chatrooms['ChatRoom']['id']);
        $this->render('/Chats/delChatting');
    }

    public function more_index($type = 1, $query = null, $page, $limit = LIMIT_DSIPLAY_ANIMAL)
    {
        $this->loadModel('User');
        if($type == 1)
        {
            $users = $this->User->getUsers($page);
            $more_users = $this->User->getUsers( $page + 1);
        }
        else if($type == 2)//search
        {
            $users = $this->User->getSearchResult($query, $page);
            $more_users = $this->User->getSearchResult( $query, $page + 1);
        }
        $more_result = 0;
        if(count($more_users) > 0)
            $more_result = 1;
        if(empty($query))
            $query = ".,";
        $this->set('users', $users);
        $this->set('more_result', $more_result);
        $this->set('more_users_url', '/chat/more_index/' . $type . '/' . $query .'/' . ( $page + 1 ) . '/' . $limit );
        $this->render('/Elements/lists/chat_users');
    }

    public function index($type = 1, $query = null, $page = 1, $limit = LIMIT_DSIPLAY_ANIMAL )
    {
        $this->loadModel('User');
        if($type == 1)
        {
    ...
    public function messages($roomId = 0){
        ...
        // Here is lists of user for all room
