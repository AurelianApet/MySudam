...
class Friend extends AppModel {
	...
	/*
	 * Return a lists of friends for dropdown lists
	 * @param int $uid
	...
	public function getFriendsList( $uid, $excludes = array() )
	{
		...
		$friends = $this->find( 'all', array( 'conditions' => $cond, 
											  ...
											  'order'	   => 'User.name asc'
							) 	); // have to do this because find(lists) does not work with bindModel
	...
	public function getFriends( $uid )
	{
		...
		$friends = $this->find( 'lists' , array( 'conditions' => array( 'Friend.user_id' => $uid ),
	...
	/*
	 * Return a lists of friends for displaying
	 */
    ...
    /*
     * Return a lists of friends for searching
     */
	 ...
	 * @param int $uid
	 * @param boolean $bigList - view all lists or not (right column block)
	...
	public function getFriendSuggestions($uid, $bigList = false, $limit = 2) {
            ...
            if (!empty($friends)) {
                ...
                $requests = $req->find('lists', array('conditions' => array('FriendRequest.sender_id' => $uid),
                    'fields' => array('FriendRequest.user_id')
                        ));
                $be_requests = $req->find('lists', array('conditions' => array('FriendRequest.user_id' => $uid),
                    'fields' => array('FriendRequest.sender_id')
                        ));
                
                // merge with friends lists
                $userBlockModal = MooCore::getInstance()->getModel('UserBlock');               
                $blockedUsers = $userBlockModal->getBlockedUsers();
