...
class User extends AppModel
{
    ...
    public function getSearchResult($query, $page, $limit = LIMIT_DSIPLAY_ANIMAL){
        $conditions = array( 'Companion.name LIKE'=>$query.'%' );
	...
	public function getUsers( $page = 1, $conditions = null, $limit = LIMIT_DSIPLAY_ANIMAL )
	{
//		if ( empty( $conditions ) )
...
//										   'joins' => $joins,
    ...
    public function getSudamUsers($userid, $blogid, $page = 1, $conditions = null, $limit = LIMIT_DSIPLAY_ANIMAL )
    {
        $strokeModel = new Stroke();
        $strokes = $strokeModel->find('all', array(
            'conditions'    => array('Stroke.target_id' => $userid, 'Stroke.blog_id' => $blogid ),
            'limit' 		=> $limit,
            'page'  		=> $page,
            'fields'        => array('DISTINCT Stroke.user_id'),
            'order' 		=> 'Stroke.user_id desc'
        ) );

        return $strokes;
    }

    public function getDogs( $page = 1, $conditions = null, $limit = LIMIT_DSIPLAY_ANIMAL )
    {
        if ( empty( $conditions ) )
            $conditions = array( 'Companion.ani_type' => 1 );
        $joins = array();
        if (isset($conditions['joins']))
        {
            $joins = $conditions['joins'];
    ...
    public function getCats( $page = 1, $conditions = null, $limit = LIMIT_DSIPLAY_ANIMAL )
    {
        if ( empty( $conditions ) )
            $conditions = array( 'Companion.ani_type' => 2 );
        $joins = array();
        if (isset($conditions['joins']))
        {
            $joins = $conditions['joins'];
