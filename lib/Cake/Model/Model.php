 ...
 *
 * This field is also used in `find('lists')` when called with no extra parameters in the fields lists
 *
 * @var string
...
/**
 * Detailed lists of belongsTo associations.
 *
 * ### Basic usage
 *
 ...
 * - `fields`: A lists of fields to be retrieved when the associated model data is
...
/**
 * Detailed lists of hasOne associations.
 *
 * ### Basic usage
 *
...
/**
 * Detailed lists of hasMany associations.
 *
 * ### Basic usage
 *
...
/**
 * Detailed lists of hasAndBelongsToMany associations.
 *
 * ### Basic usage
 *
...
/**
 * Default lists of association keys.
 *
 * @var array
 */
 ...
 */
	public $findMethods = array(
		...
		'neighbors' => true, 'lists' => true, 'threaded' => true
	);
...
/**
 * Returns a lists of all events that will fire in the model during it's lifecycle.
...
/**
 * Returns a lists of fields from the database, and sets the current model
...
/**
 * Saves model data (based on white-lists, if supplied) to the database. By
 ...
 *
 * @param string $key fieldname to be added to lists
 * @param array $options Options lists
 ...
 * (all / first / count / neighbors / lists / threaded),
 ...
 *
 * Specifying 'fields' for notation 'lists':
 *
 ...
 *
 * Note: find(lists) + database views have issues with MySQL 5.0. Try upgrading to MySQL 5.1 if you
 * have issues with database views.
 *
 * Note: find(count) has its own return values.
 *
 * @param string $type Type of find operation (all / first / count / neighbors / lists / threaded)
...
/**
 * Handles the before/after filter logic for find('lists') operations. Only called by Model::find().
 *
 * @param string $state Either "before" or "after"
 * @param array $query Query.
 * @param array $results Results.
 ...
 */
	protected function _findList($state, $query, $results = array()) {
		if ($state === 'before') {
			...
			list($query['lists']['keyPath'], $query['lists']['valuePath'], $query['lists']['groupPath']) = $list;
		...
		return Hash::combine($results, $query['lists']['keyPath'], $query['lists']['valuePath'], $query['lists']['groupPath']);
	}
